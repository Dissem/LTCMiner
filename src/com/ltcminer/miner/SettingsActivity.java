...
public class SettingsActivity extends Activity implements Constants {
	...
	public void loadDefault() {
		...
		Toast.makeText(getBaseContext(), "Defaults Loaded", Toast.LENGTH_SHORT)
				.show();
	}
	...
	public void saveSettings() {
		...
		if (verify() == true) {
			...
			sb.setLength(0);
			int threads = Integer.parseInt(sb.append(et_thread.getText())
					.toString());
			sb.setLength(0);
			float throttle = (float) sb_throttle.getProgress() / 100;
			sb.setLength(0);
			long scantime = Long.parseLong(sb.append(et_scanTime.getText())
					.toString());
			sb.setLength(0);
			long retrypause = Long.parseLong(sb.append(et_retryPause.getText())
					.toString());
			...
			Log.i("LC",
					"Settings: Pri " + (String) spn_priority.getSelectedItem());
			...
			Toast.makeText(getBaseContext(), "Settings: Errors changed to red",
					Toast.LENGTH_SHORT).show();
		}
		editor.commit();
	}
	...
	public void loadSettings() {
		...
		et_thread.setText(sb.append(
				settings.getInt(PREF_THREAD, DEFAULT_THREAD)).toString());
		sb.setLength(0);
		sb_throttle.setProgress((int) (settings.getFloat(PREF_THROTTLE,
				DEFAULT_THROTTLE) * 100));
		et_scanTime.setText(sb.append(
				settings.getLong(PREF_SCANTIME, DEFAULT_SCANTIME)).toString());
		sb.setLength(0);
		et_retryPause.setText(sb.append(
		...
		Toast.makeText(getBaseContext(), "Settings Loaded", Toast.LENGTH_SHORT)
				.show();
	}
	...
	public boolean verify() {
		...
		if (et_retryPause.getText() == null) {
			...
			if (scanTime < 1) {
				Log.i("LC",
						"Settings Error: retry pause must be greater than 1ms");
				et_retryPause.setShadowLayer(1, 1, 1, Color.RED);
				success = false;
			} else {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		spn_priority.setAdapter(adapter2);
		spn_priority.setSelection(1);
